<script lang="ts">
	...
	let averageRating = $derived.by(() => {
		...
		const sumOfAllRatings = booksWithRating.reduce((avg, book, i) => avg + book.rating!, 0);
		return Math.round(100 * (sumOfAllRatings / booksWithRating.length)) / 100; // Round to 2 decimals
	});
